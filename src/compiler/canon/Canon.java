...
public class Canon {
	...
	static boolean isNop(Stm a) {
		return a instanceof EXPE && ((EXPE) a).exp instanceof CONST;
	}
	...
	static Stm do_stm(Stm s) {
		...
		else if (s instanceof EXPE)
			return do_stm((EXPE) s);
		else
			return reorder_stm(s);
	}
	...
	static StmExpList nopNull = new StmExpList(new EXPE(new CONST(0)), null);
		...
		if (exps == null)
			...
			if (a instanceof CALL) {
				...
				if (commute(bb.stm, aa.exp))
					...
					Temp t = new Temp();
					return new StmExpList(seq(aa.stm,
							seq(new MOVE(new TEMP(t), aa.exp), bb.stm)),
