/* Generated By:JavaCC: Do not edit this line. MiniJavaParserConstants.java */
package compiler.javacc;

/**
 * Token literal values and constants. Generated by
 * org.javacc.parser.OtherFilesGen#start()
 */
public interface MiniJavaParserConstants {

	/** End of File. */
	int EOF = 0;
	/** RegularExpression Id. */
	int CLASS = 1;
	/** RegularExpression Id. */
	int WHILE = 2;
	/** RegularExpression Id. */
	int PUBLIC = 3;
	/** RegularExpression Id. */
	int STATIC = 4;
	/** RegularExpression Id. */
	int VOID = 5;
	/** RegularExpression Id. */
	int MAIN = 6;
	/** RegularExpression Id. */
	int EXTENDS = 7;
	/** RegularExpression Id. */
	int RETURN = 8;
	/** RegularExpression Id. */
	int OPNOT = 9;
	/** RegularExpression Id. */
	int OPAND = 10;
	/** RegularExpression Id. */
	int OPLT = 11;
	/** RegularExpression Id. */
	int OPPLUS = 12;
	/** RegularExpression Id. */
	int OPMINUS = 13;
	/** RegularExpression Id. */
	int OPTIMES = 14;
	/** RegularExpression Id. */
	int NEW = 15;
	/** RegularExpression Id. */
	int THIS = 16;
	/** RegularExpression Id. */
	int LENGTH = 17;
	/** RegularExpression Id. */
	int STRING = 18;
	/** RegularExpression Id. */
	int INT = 19;
	/** RegularExpression Id. */
	int BOOLEAN = 20;
	/** RegularExpression Id. */
	int TRUE = 21;
	/** RegularExpression Id. */
	int FALSE = 22;
	/** RegularExpression Id. */
	int IF = 23;
	/** RegularExpression Id. */
	int ELSE = 24;
	/** RegularExpression Id. */
	int DIGIT = 25;
	/** RegularExpression Id. */
	int CHARACTER = 26;
	/** RegularExpression Id. */
	int ID = 27;
	/** RegularExpression Id. */
	int NUM = 28;
	/** RegularExpression Id. */
	int REAL = 29;
	/** RegularExpression Id. */
	int INTEGER_LITERAL = 30;

	/** Lexical state. */
	int DEFAULT = 0;

	/** Literal token values. */
	String[] tokenImage = { "<EOF>", "\"class\"", "\"while\"", "\"public\"",
			"\"static\"", "\"void\"", "\"main\"", "\"extends\"", "\"return\"",
			"\"!\"", "\"&&\"", "\"<\"", "\"+\"", "\"-\"", "\"*\"", "\"new\"",
			"\"this\"", "\"length\"", "\"String\"", "\"int\"", "\"boolean\"",
			"\"true\"", "\"false\"", "\"if\"", "\"else\"", "<DIGIT>",
			"<CHARACTER>", "<ID>", "<NUM>", "<REAL>", "<INTEGER_LITERAL>",
			"<token of kind 31>", "\" \"", "\"\\t\"", "\"\\n\"", "\"{\"",
			"\"(\"", "\"[\"", "\"]\"", "\")\"", "\"}\"", "\";\"", "\",\"",
			"\"System.out.println\"", "\"=\"", "\".\"", };

}
