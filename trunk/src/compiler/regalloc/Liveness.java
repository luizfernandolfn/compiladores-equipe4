...
public class Liveness extends InterferenceGraph {
  ...
  private Node[] l2a(NodeList nl) {
    int n=0;
    for(NodeList p=nl; p!=null; p=p.tail) n++;
  ...
  public Liveness(compiler.flowgraph.AssemFlowGraph afg) {
    ...
    int it=1;
    print("use",use); print("def",def);
    while (changed) {
      changed = false;
      if (debug) {
	System.out.println("----------------"+it+"------------");
      	print("in",liveIn); print("out",liveOut);
      }
  ...
  private void addInterferenceEdges(compiler.flowgraph.AssemFlowGraph flow, Node flownode,
			    Temp deftemp, Temp livetemp) {
    Temp[] s = flow.use(flownode);
  ...
  public MoveList moves(){ return movelist; }
}
